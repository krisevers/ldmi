...
class LBR:
    def __init__(self, E):
        ...
        depths = np.linspace(0,100,2*E['K']+1) # Normalized distance to the center of individual depths (in %)
        self.P['l']    = depths[1::2]
    ...
    def sim(self, cbf, E, *args):
        ...
        #------------------------------------------------------
        K = int(E['K'])		# Number of depths

        # BASELINE PARAMETERS
        V0t 	= np.float128(self.P['V0t'])		    # Total amount of CBV0 within GM tissue (in mL)
        V0t_p	= np.float128(self.P['V0t_p'])	        # Total amount of CBV0 in pial vein (in mL)

        w_v		= np.float128(self.P['w_v'])		    # Fraction of CBV0 in venules with respect to the total
        w_d  	= 1-w_v			                        # Fraction of CBV0 in ascending vein with respect to the total

        s_v 	= np.float128(self.P['s_v'])		    # Slope of CBV0 increase towards the surface in venules
        s_d  	= np.float128(self.P['s_d'])		    # Slope of CBV0 increase towards the surface in ascending veins

        # Depth-specific CBV0
        if len(self.P['x_v']) == K:                     # For venules
            x_v  = self.P['x_v'].astype(np.float128)    # Depth-specific fractions defined by user
        else:
            x_v  = (10+s_v*np.flipud(self.P['l'])).astype(np.float128) # Possibility to define linear increase (default s_v = 0)
        x_v      = x_v/np.sum(x_v)                      # Fraction of CBV0 across depths in venules 

        if len(self.P['x_v']) == K:                     # For ascending vein
            x_d  = self.P['x_d'].astype(np.float128)    # Depth-specific fractions defined by user
        else:
            x_d  = 10+s_d*np.flipud(self.P['l']).astype(np.float128) # Possibility to define linear increase 
        ...
        if hasattr(self.P['t0v'], '__len__'):
            t0v = self.P['t0v'].astype(np.float128)
        else:
            t0v = (np.ones(K)*self.P['t0v']).astype(np.float128)
        ...
        if hasattr(self.P['E0v'], '__len__'):
            E0v        = self.P['E0v'].astype(np.float128)     # depth-specific defined by user
        else:
            E0v        = (np.ones(K)*self.P['E0v']).astype(np.float128)
        if hasattr(self.P['E0d'], '__len__'):
            E0d        = self.P['E0d'].astype(np.float128)      # depth-specific defined by user
        else:
            E0d        = (np.ones(K)*self.P['E0d']).astype(np.float128)
        E0p        = np.float128(self.P['E0p'])
        ...
        if hasattr(self.P['n'], '__len__'):      # For venules (microvasculature)
            n      = np.float128(self.P['n'])    # Depth-specific defined by user
        else:
            n      = (np.ones(K)*self.P['n']).astype(np.float128)                # Default

        # Grubb's exponent alpha (i.e CBF-CBV steady-state relationship)
        if hasattr(self.P['alpha_v'], '__len__'):		                        # For venules
            alpha_v    = self.P['alpha_v'].astype(np.float128)       		    # Depth-specific defined by user 
        else:
            alpha_v    = (np.ones(K)*self.P['alpha_v']).astype(np.float128)  	# Default
        if hasattr(self.P['alpha_d'], '__len__'):		                        # For ascending vein
            alpha_d    = self.P['alpha_d'].astype(np.float128)             	    # Depth-specific defined by user  
        else:
            alpha_d    = (np.ones(K)*self.P['alpha_d']).astype(np.float128)  	# Default
        alpha_p        = np.float128(self.P['alpha_p'])      			        # For pial vein

        # CBF-CBV uncoupling (tau) during inflation and deflation:
        if hasattr(self.P['tau_v_in'], '__len__'):   	                        # For venules (inflation)
            tau_v_in  = self.P['tau_v_in'].astype(np.float128)             	    # Depth-specific defined by user
        else:
            tau_v_in  = (np.ones(K)*self.P['tau_v_in']).astype(np.float128) 	    # Default  
        if hasattr(self.P['tau_v_de'], '__len__'):   	                        # For venules (deflation)
            tau_v_de  = self.P['tau_v_de'].astype(np.float128)             	    # Depth-specific defined by user  
        else:
            tau_v_de  = (np.ones(K)*self.P['tau_v_de']).astype(np.float128)  	# Default  
        if hasattr(self.P['tau_d_in'], '__len__'):   	                        # For ascending vein (inflation)
            tau_d_in  = np.float128(self.P['tau_d_in']).astype(np.float128)	    # Depth-specific defined by user 
        else:
            tau_d_in  = (np.ones(K)*self.P['tau_d_in']).astype(np.float128)   	# Default  
        if hasattr(self.P['tau_d_de'], '__len__'):   	                        # For ascending vein (deflation)
            tau_d_de  = self.P['tau_d_de'].astype(np.float128)             	    # Depth-specific defined by user 
        else:
            tau_d_de  = (np.ones(K)*self.P['tau_d_de']).astype(np.float128)  	# Default
        tau_p_in      = np.float128(self.P['tau_p_in'])       		            # For pial vein (inflation)
        tau_p_de      = np.float128(self.P['tau_p_de'])       		            # For pial vein (deflation)
        ...
        TE     = np.float128(self.P['TE'])	 	 # echo-time (sec) 

        Hct_v  = np.float128(self.P['Hct_v'])	 # Hematocrit fraction
        Hct_d  = np.float128(self.P['Hct_d'])
        Hct_p  = np.float128(self.P['Hct_p'])
        B0     = np.float128(self.P['B0'])   	 # Field strenght        
        gyro   = np.float128(self.P['gyro'])      # Gyromagnetic constant 
        suscep = np.float128(self.P['suscep'])    # Susceptibility difference
        ...
        # Water proton density 
        rho_t  = np.float128(self.P['rho_t'])  # In GM tissue
        rho_v  = np.float128(self.P['rho_v'])  # In blood (venules) Ref. Lu et al. (2002) NeuroImage
        rho_d  = np.float128(self.P['rho_d'])  # In blood (ascening vein) 
        rho_p  = np.float128(self.P['rho_p'])  # In blood (pial vein) 
        rho_tp = np.float128(self.P['rho_tp']) # In in tissue and CSF 

        # Relaxation rates (in sec-1):
        if hasattr(self.P['R2s_t'],'__len__'):   	# For tissue
            R2s_t  = self.P['R2s_t'].astype(np.float128)
        else:
            R2s_t  = (np.ones(K)*self.P['R2s_t']).astype(np.float128)   	# (sec-1)
        if hasattr(self.P['R2s_v'],'__len__'):		# For venules
            R2s_v  = self.P['R2s_v'].astype(np.float128)               	# (sec-1)
        else:
            R2s_v  = (np.ones(K)*self.P['R2s_v']).astype(np.float128)  	# (sec-1) 
        if hasattr(self.P['R2s_d'],'__len__'): 		# For ascening vein
            R2s_d  = self.P['R2s_d'].astype(np.float128)           		# (sec-1)
        else:
            R2s_d  = (np.ones(K)*self.P['R2s_d']).astype(np.float128) 	# (sec-1)  
        R2s_p  = np.float128(self.P['R2s_p'])         			# For pial vein 
        ...
        LBR       = np.zeros((int(E['T']/dt),K)).astype(np.float128)
        LBRpial   = np.zeros((int(E['T']/dt),K)).astype(np.float128)
