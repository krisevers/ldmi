...
if __name__=="__main__":
    ...
    num_areas = len(areas)                  # number of areas
    num_columns = [2, 2]                    # number of columns per area
    tot_num_columns = np.sum(num_columns)   # total number of columns
    M = 8                                   # number of populations per area
    tot_num_pops = tot_num_columns * M      # total number of populations
    ...
    # within area
    mask[M:M*2, :M]             = 1 # V1_V -> V1_H
    mask[:M, M:M*2]             = 1 # V1_H -> V1_V
    mask[M*3:, M*2:M*3]         = 2 # MT_V -> MT_H
    mask[M*2:M*3, M*3:]         = 2 # MT_H -> MT_V
    # feedforward
    mask[M*2:M*3, :M]           = 3 # V1_V -> MT_V
    mask[M*3:, M:M*2]           = 3 # V1_H -> MT_H
    mask[M*3:, :M]              = 4 # V1_V -> MT_H
    mask[M*2:M*3, M:M*2]        = 4 # V1_H -> MT_V
    # feedback
    mask[:M, M*2:M*3]           = 5 # MT_V -> V1_V
    mask[M:M*2, M*3:]           = 5 # MT_H -> V1_H
    mask[:M, M*3:]              = 6 # MT_H -> V1_V
    mask[M:M*2, M*2:M*3]        = 6 # MT_V -> V1_H
    ...
    plt.xticks(np.arange(M/2, tot_num_pops, M), [r'$V1_V$', r'$V1_H$', r'$MT_V$', r'$MT_H$'])
    plt.yticks(np.arange(M/2, tot_num_pops, M), [r'$V1_V$', r'$V1_H$', r'$MT_V$', r'$MT_H$'])
    ...
    I_V1 = np.tile(np.random.randn(M), 2)
    I_MT = np.tile(np.random.randn(M), 2)
    I_ext = np.concatenate((I_V1, I_MT))
    I_ext[I_ext < 0.75] = 0
    I_ext *= 1e-3

    X, Y, I, eigs = DMF_MA(areas=areas, num_columns=num_columns, A=A, I_ext=I_ext, G=1)
    ...
    # MT_V
    plt.subplot(2, 2, 3)
    plt.imshow(X[:, M*2:M*3], cmap='Reds', aspect='auto', interpolation='none')
    plt.xticks([])
    plt.yticks([])
    plt.ylabel('Time (ms)')
    plt.xlabel('Population')
    ...
    # MT_H
    plt.subplot(2, 2, 4)
    plt.imshow(X[:, M*3:], cmap='Reds', aspect='auto', interpolation='none')
    plt.xticks([])
    plt.yticks([])
    plt.ylabel('Time (ms)')
    plt.xlabel('Population')
    ...
    for i, area in enumerate(all_areas):
        PROB_K = I2K(K, 'macaque', area, sigma=5)
    ...
    plt.title(r'$V1_H$')
    plt.subplot(2, 2, 3)
    plt.imshow(MAP[:, 2], cmap='Reds', aspect='auto', interpolation='none')
    plt.xticks([])
    plt.yticks([])
    plt.ylabel('Time (ms)')
    plt.xlabel('Depth')
    plt.title(r'$MT_V$')
    plt.subplot(2, 2, 4)
    plt.imshow(MAP[:, 3], cmap='Reds', aspect='auto', interpolation='none')
    plt.xticks([])
    plt.yticks([])
    plt.ylabel('Time (ms)')
    plt.xlabel('Depth')
    ...
    plt.figure()
    dt = 1e-4
    plt.plot(MAP[int(0.5/dt), 0] - MAP[int(0.1/dt), 0], label=r'$V1_V$', color='C0')
    plt.plot(MAP[int(0.5/dt), 1] - MAP[int(0.1/dt), 1], label=r'$V1_H$', color='C1')
    plt.plot(MAP[int(0.5/dt), 2] - MAP[int(0.1/dt), 2], label=r'$MT_V$', color='C2')
    plt.plot(MAP[int(0.5/dt), 3] - MAP[int(0.1/dt), 3], label=r'$MT_H$', color='C3')
    plt.legend()
    plt.xlabel('Depth')
    plt.ylabel('Current')
    plt.show()
